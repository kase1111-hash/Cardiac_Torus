...
function buildECG(rr) {
  ...
  for (let i = 0; i < rr.length; i++) {
    ...
    for (let j = 0; j < samples; j++) {
      const x = j/samples, y = 0.12*Math.exp(-((x-0.08)/0.03)**2)
        - 0.08*Math.exp(-((x-0.14)/0.008)**2) + Math.exp(-((x-0.155)/0.012)**2)
        - 0.18*Math.exp(-((x-0.17)/0.01)**2) + 0.25*Math.exp(-((x-0.32)/0.055)**2)
...
function buildPhono(beats) {
  ...
  for (const b of beats) {
    ...
    for (let j = 0; j < samples; j++) {
      const x = j/samples;
      let y = b.s1 * Math.exp(-((x-0.05)/0.02)**2) * Math.sin(TWO_PI*x*15)
        + b.s2 * Math.exp(-((x-0.35)/0.02)**2) * Math.sin(TWO_PI*x*12);
      // Add murmur energy in systole
      if (b.sysE > 0.025) {
        y += (b.sysE * 8) * Math.exp(-((x-0.2)/0.1)**2) * Math.sin(TWO_PI*x*25 + Math.random());
      }
...
function SubstratePanel({ conditions, mode, buildWaveform, torusExtractor, signalLabel, torusLabel, waveColor }) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (torRef.current) {
        ...
        drawTorus(ctx, 340, 340, pts, ks, 3, torusLabel, 60);
      }
      ...
      if (sigRef.current && data.signal.length > 0) {
        ...
        drawSignal(ctx, 440, 200, vis, maxT, signalLabel, waveColor);
      }
      
      if (show < maxPts) frameRef.current = requestAnimationFrame(animate);
    };
  ...
  return (
    <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
      <div>
        <canvas ref={sigRef} width={440} height={200} style={{ borderRadius: 8, border: '1px solid #1a2540' }} />
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8, maxWidth: 440 }}>
          {conditions.map((cc, i) => (
      ...
      <div>
        <canvas ref={torRef} width={340} height={340} style={{ borderRadius: 8, border: '1px solid #1a2540' }} />
